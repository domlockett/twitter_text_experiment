...
def prep_csv(file_path, colnames):
    with open(file_path, mode = "w") as f:
        w = csv.DictWriter(f, fieldnames = colnames)
        w.writeheader()

def write_tweet_data(json_obj, colnames, file_path, tweet_id = None):
    df = pandas.json_normalize(json_obj)
    ...
    df.to_csv(file_path, index=False, mode='a', columns=colnames, header = 0)
...
def get_user_tweets(headers, filename, user_ids, start_time, end_time):
    ...
    prep_csv(file_path = 'data/tweets/tweets ' + filename, colnames = tweet_colnames)

    # create structure of Media-level csv
    media_colnames = ['media_key', 'type', 'duration_ms', 'preview_image_url', 'public_metrics.view_count']
    prep_csv(file_path = 'data/tweets-media/tweets-media ' + filename, colnames = media_colnames)

    # create structure of context csv
    context_colnames = ['tweet_id', 'domain.id', 'domain.name', 'entity.id', 'entity.name']
    prep_csv(file_path = 'data/tweets-context/tweets-context ' + filename, colnames = context_colnames)
    ...
    for i in user_ids:
        ...
        while stop == False:
            ...
            params_user_tweets = get_params_user_tweets(user_id = 28785486,
            ...
            write_tweet_data(json_obj = json_response["data"], colnames = tweet_colnames, file_path = 'data/tweets/tweets ' + filename)

            # then, write Media-level data to file
            if "includes" in json_response.keys():
                write_tweet_data(json_obj = json_response["includes"]["media"], colnames = media_colnames, file_path = 'data/tweets-media/tweets-media ' + filename)

            # then, write context data to file
            for j in json_response["data"]:
                if "context_annotations" in j.keys():
                    write_tweet_data(json_obj = j["context_annotations"], colnames = context_colnames, file_path = 'data/tweets-context/tweets-context ' + filename, tweet_id = j["id"])
...
def main():
    ...
    logging.info('Scraping: %s through %s', start_time, end_time)
    ...
    get_user_tweets(headers, filename, user_ids, start_time, end_time)
